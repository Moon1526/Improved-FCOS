...
class FCOSLossComputation(object):
    ...
    def compute_centerness_targets(self, reg_targets, pred, target):
        '''
        计算centerness真值 （对应FCOS论文公式(3)），加入IOU
        :param reg_targets: （FPN层数*batch*H*W,4）
        :return: centerness真值
        '''
        ...
        gious =((ious - (ac_uion - area_union) / ac_uion)+1)/2
    ...
    def __call__(self, locations, box_cls, box_regression, centerness, targets):
        ...
        if pos_inds.numel() > 0:
            centerness_targets = self.compute_centerness_targets(reg_targets_flatten, box_regression_flatten,
                                                                 reg_targets_flatten)
