...
def round_filters(filters, global_params):
    """ Calculate and round number （取整数） of filters based on depth multiplier. """
    # 对应模型Di的i值
    ...
    filters *= multiplier
    # min_depth=None时，赋值min_depth=divisor=8
...
def drop_connect(inputs, p, training):
    """ Drop connect. (input在drop rate参数p的控制下，百分比降低值并去掉比例p的值，针对输入)"""
    if not training: return inputs
    batch_size = inputs.shape[0]
    keep_prob = 1 - p
    random_tensor = keep_prob
    # torch.rand为(0,1)均匀分布中抽取数字
    # random_tensor的值域也即(1-p,2-p)
    random_tensor += torch.rand([batch_size, 1, 1, 1], dtype=inputs.dtype, device=inputs.device)
    # torch.floor,返回一个新张量,包含输入input张量每个元素的floor,即不大于元素的最大整数。
    # random_tensor的值为(1-p,1)的变为：0（占比=p）；值为[1,2-p)的变为：1
    binary_tensor = torch.floor(random_tensor)
    # inputs / keep_prob是因为input被drop了p，因此整体值都按p比例下降
...
class BlockDecoder(object):
    """ Block Decoder for readability, straight from the official TensorFlow repository （把参数从string组装为dict）"""
    ...
    def decode(string_list):
        ...
        （BlockArgs超参数组装/格式化，string->dict）

        :param string_list: a list of strings, each string is a notation of block
        (Bo的stage2-stage8的网络超参数)
...
def efficientnet(width_coefficient=None, depth_coefficient=None, dropout_rate=0.2,
    ...
    """ Creates a efficientnet model.(组装efficientnet的输入参数blocks_args(基于B0变化), global_params(固定训练超参)) """
